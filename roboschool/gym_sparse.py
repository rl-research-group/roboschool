...
class RoboschoolSparseInvertedDoublePendulum(RoboschoolMujocoXmlEnv):
    def __init__(self):
    ...
    def _step(self, a):
        ...
        dist_penalty = 0 #0.01 * self.pos_x ** 2 + (self.pos_y + 0.3 - 2) ** 2
        ...
        self.rewards = [1 if self.pos_y + 0.3 <= 1 else 0] #[float(alive_bonus), float(-dist_penalty), float(-vel_penalty)]
        self.frame  += 1
        self.done   += False   # 2 == 1+True
...
class RoboschoolSparseInvertedPendulum(RoboschoolMujocoXmlEnv):
    swingup = False
    ...
    def _step(self, a):
        ...
        if self.swingup:
            ...
            reward = 1 if np.abs(self.theta) > .2 else 0
            done = False
